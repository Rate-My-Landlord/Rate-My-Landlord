...
def resolve_new_review(obj, info, landlord_id, overall_star_rating,
                       ...
                       entry_without_notice=None, cost_of_rent_rating=None,
                       text=None):
    """Write a new review"""
    try:
        if not Landlord.landlord_exists(landlord_id):
            raise Exception(
                'No landlord found with id of {}'.format(landlord_id))
        ...
        if user.id != int(author_id):
            raise Exception(
                'User id of token does not match provided author id')
        ...
        new_review = Review(landlord_id=landlord_id,
                            ...
                            entry_without_notice=entry_without_notice,
                            cost_of_rent_rating=cost_of_rent_rating,
                            text=text)
